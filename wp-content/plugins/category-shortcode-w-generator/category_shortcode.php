	...
	switch ($rd_loop_method) {
    		case 'excerpt': 		
 			foreach($rdcsc_posts as $post) :
				...
				$rdscf_output.='<div class=csc_post csc_excerpt><a href="' . get_permalink($rdcsc_posts ->ID).'">';
			
				/// thumbnail added in 1.2 release
				$rdscf_output.='<div class=csc_post_thumbnail>' . get_the_post_thumbnail($rdcsc_posts->ID, 'full') . '</div>';
				
				$rdscf_output.='<div class="heading clearfix"><div class=csc_post_title><h2 class=post-title>' . the_title("", "", false) .'</h2></div>';
				$rdscf_output.='<div class=csc_post_date><h2 class=post-title>' . the_date('','','',FALSE) .'</h2></div></div>';
			...
			foreach($rdcsc_posts as $post) :
				...
				$rdscf_output.='<div class="heading clearfix"><div class=csc_post_title><h2 class=post-title><a href="' . get_permalink($rdcsc_posts ->ID).'">' . the_title("", "", false) .'</a></h2></div>';
				$rdscf_output.='<div class=csc_post_date><h2 class=post-title>' . the_date('','','',FALSE) .'</h2></div></div>';
				//$rdscf_output.='<div class=csc_post_author>' . $rdcsc_author . '</div>';
				//foreach((get_the_category()) as $category) { 
					//$rdscf_output.='<div class=csc_post_category>' . $category->cat_name . '</div>'; 
...
<div class='wrap'>
	...
	<div class='form-field'>
		<span class='label'><label for='rdcsc_number'></label></span>
			<input type='text' class='numberofposts' name='rdcsc_admin_number' style='width: 200px'/>
	</div>
	<div class='form-field'>
			<span class='label'><label for='rdcsc_order'></label></span>
	...
	<div class='form-field'>
			<span class='label'><label for='rdcsc_method'></label></span>
	...
	<div class="form-field">
			<span class='label'><label for='rdcsc_orderby'></label></span>
	...
	<div class='form-field'>
			<span class='label'><label for='rdcsc_category'></label></span>
